...
If the updated value is above 1 or below 0.1, it is capped to 1 and 0.1 
respectively.
...
learning; 3 = Reset associative weights to initial state, but keep
attentional strengths in alpha (see above); 4 = Reset attentional strengths
to initial state, but keep association weights (see above). 
...
absence of the outcome, although slpMK75 supports any real values for lambda.
...
\note{
  ...
  is probably the simplest way of how one can express Mackintosh's somewhat
  ...
  2. If you want to run a simulation with two possible outcomes, you will
  need to run separate simulations for each outcome at the moment. You can also
  consider using 1 and -1. This will mathematically be the same as running
  two separate simulations for outcome 1 and outcome 2 respectively. An 
  additional feature that enables the use of multiple outcome/feedback columns
  is in development.
}
